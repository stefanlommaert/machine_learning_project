...
# print(simplify_info_key_fcpa("[Round 0][Player: 0][Pot: 40000][Money: 19850 0][Private: TsTd][Public: ][Sequences: r20000]"))    
# print(simplify_card_string("[Private: TsTd]"))  
...
class ExternalSamplingSolver(mccfr.MCCFRSolverBase):
  ...
  def iteration(self):
    ...
    player.
    """
    for player in range(self._num_players):  #ga om de beurt 2 spelers af en start voor elke speler een nieuwe game eerst gaan we update met player 1 zijn regrets up te daten daarna mag player
      
      self._update_regrets(self._game.new_initial_state(), player)
      #gaan eigenlijk "player" (1 of 2) zijn regrets laten updaten en de andere speler speelt gewoon met zijn current policy dus beurt steeds doorgeven 
  ...
  def _full_update_average(self, state, reach_probs):
    ...
    #translate the state key into our wayyyy simpeler state key
    info_state_key = simplify_info_key_fcpa(state.information_state_string(cur_player))
  ...
  def _update_regrets(self, state, player):
    ...
    cur_player = state.current_player()
    #translate our info state into a wayyyy simpler state 
    info_state_key = simplify_info_key_fcpa(cur_player,state )
      
    legal_actions = state.legal_actions()
    num_legal_actions = len(legal_actions)
    # #print("CURRENT STATE PLAYER: ",cur_player)
    # #print("CURRENT UPDATE REGET PLAYER: ",player)
    #print("CURRENT INFOSTATE: ",state.information_state_string(cur_player))
    #print("SIMPLIFIED INFO STATE KEY : ",info_state_key)
    
    #ZOEK Naar de INFOSTATE info IN INFOSTATES  ._lookup_infostate_info = haal infoste uit dic als die er nog ni instaat zet hem erbij 
    infostate_info = self._lookup_infostate_info(info_state_key,
                                                 num_legal_actions)
    
    #INFOSTATE heeft per INFOSTATE een tuple van:
#     REGRET_INDEX = 0
# AVG_POLICY_INDEX = 1
# eerste zijn simpleweg de regrets gaat dus ook erst allemaal 0 zijn en moeten meerdere keren gespeeld worden en de AVG POLICY (later updaten) bepaald wat er gaat gespeeld worden 
#policy zal eerst dus gewoon tottaaaal random zijn     50/50 
    policy = self._regret_matching(infostate_info[mccfr.REGRET_INDEX],
                                   num_legal_actions)
    
    value = 0
    child_values = np.zeros(num_legal_actions, dtype=np.float64)
    if cur_player != player: #het is in de state aan de andere speler ?! dus geven beurt door denk ik 
        
      # Sample at opponent node
      
      #!!tegenstander vraagt naar wat ik zou doen als hij een state kiest!!
      
      #neem een random action volgens de curr player zijn policy 
      action_idx = np.random.choice(np.arange(num_legal_actions), p=policy) #DEZE kunnen we wel beter gebruiken voor onze agent lol perform gewoon policy!!!!!! 
      ...
      for action_idx in range(num_legal_actions):
        #wat zou de tegenstaner doen als ik action_inx speel 
        child_values[action_idx] = self._update_regrets(
            state.child(legal_actions[action_idx]), player)
        #total utility = kans op action te spelen * utility van die action eigenlijk te spelen
        value += policy[action_idx] * child_values[action_idx]
    
    if cur_player == player:
      # Update regrets.
      #natuurlijk we zijn player 1 allemaal aan het doen anders was het waardeloos
      for action_idx in range(num_legal_actions):
    ...
    if self._average_type == AverageType.SIMPLE and cur_player == (
      ...
      for action_idx in range(num_legal_actions):#average updaten is niet belangrijk denk ik
