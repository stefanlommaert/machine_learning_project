...
class Agent(pyspiel.Bot):
    ...
    def get_cards(self,info_key):
        ...
        privcards= info_key.split("]")[0].replace("Public: ","")[1:] 
    ...
    def step(self, state):
        ...
        policys=[]
        maxes = []
        print("ALL SUBCARDS: ",subcards)
        for subcards in all_subcards:
            sub_info_key =  self.create_valid_info_key(privcards,subcards,actionString)   
            infostate_info = self._lookup_infostate_info(sub_info_key,
                                                    num_legal_actions)
            policy = self._regret_matching(infostate_info[mccfr.REGRET_INDEX],
                                    num_legal_actions)
            
            policys+=policy
            maxes+=np.amax(policy)
