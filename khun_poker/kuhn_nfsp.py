...
def main(unused_argv):
  ...
  with tf.Session() as sess:
    ...
    for ep in range(FLAGS.num_train_episodes):
      if (ep + 1) % FLAGS.eval_every == 0 and (ep+1)>10000:
        losses = [agent.loss for agent in agents]
        logging.info("Losses: %s", losses)
        #exploitability.exploitability for exploitability
        expl = exploitability.nash_conv(env.game, expl_policies_avg)
        logging.info("[%s] NASH_CONV AVG %s", ep + 1, expl)
        logging.info("_____________________________________________")
        
        Step_values.append(ep+1)
    ...
    ax.set_xlim(10000,max(Step_values))  
